/**
 * @file as_rv32i_core.v
 * 
 * @brief Top Module for the RV32I core.
 * 
 * @author   UQAC | SHACHA
 * 
...
/// \module as_rv32i_core
///
/// \brief Top-level module for the RV32I core.
///
/// \param PC_RESET PC reset value (default = 32'h00_00_00_00)
/// \param TRAP_ADDRESS Trap address value (default = 0)
/// \param ZICSR_EXTENSION ZICSR extension flag (default = 1)
///
...
module as_rv32i_core #(parameter PC_RESET = 32'h00_00_00_00, TRAP_ADDRESS = 0, ZICSR_EXTENSION = 1) ( 
 ...
 /********************************** TO DO: Formal Verification | Yosys *********************************/
   ...
   `ifdef FORMAL 
        ...
        always @(posedge i_clk) begin
            /// change_pc in stage 5 (due to traps) will force first stage to change PC in next clk cycle and all _ce to be disabled (excecpt for fetch_ce which is always high) ///
            ...
            /// change_pc in stage 3 (due to jumps and branches) will force first stage to change PC in next clock cycle unless stalled by stage 3(due to data dependency) or stage 4(due to load instruction) or be flushed by stage 5(due to traps) and all _ce of previous stages of STAGE 3 to be disabled (except for fetch_ce which is always high) ///
        ...
        always @(posedge i_clk) begin
            ...
            if(writeback_change_pc) assert(stall == 0); ///< pipeline will never be stalled and flushed(by stage 5) at same time No stall can stop flush from stage 5
